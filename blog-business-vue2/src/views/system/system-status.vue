<template>
  ...
  <div :style="!show ? 'height: 500px' : 'height: 100%'" class="app-container">
    <div v-if="show">
      ...
      <!--        </div>-->
      <!--        <div ref="cpuInfo">-->
      <!--          &lt;!&ndash;<v-chart :options="cpuInfo" />&ndash;&gt;-->
      <!--        </div>-->
      <!--      </el-card>-->
      <!--    </el-col>-->
      <!--    <el-col :xs="24" :sm="24" :md="12" :lg="12" :xl="12" style="margin-bottom: 10px">-->
      ...
      <!--        </div>-->
      <!--        <div ref="memoryInfo">-->
      <!--          &lt;!&ndash;<v-chart :options="memoryInfo" />&ndash;&gt;-->
      <!--        </div>-->
      <!--      </el-card>-->
      <!--    </el-col>-->
      <!--  </el-row>-->
...
export default {
  ...
  //
  //  //初始化Echarts组件
  //  let cpuInfoChart = echarts.init(this.$refs.cpuInfo)
  //  cpuInfoChart.setOption(this.cpuInfo)
  //
  //  let memoryInfoChart = echarts.init(this.$refs.memoryInfo)
  //  memoryInfoChart.setOption(this.memoryInfo)
  //},
  destroyed() {
  ...
  methods: {
    init() {
      systemAPi.systemStatus().then(resp => {
        this.data = resp.data
        this.show = true
        ...
        this.cpuInfo.xAxis.data.push(resp.data.time)
        this.memoryInfo.xAxis.data.push(resp.data.time)
        this.cpuInfo.series[0].data.push(parseFloat(resp.data.cpu.used))
        this.memoryInfo.series[0].data.push(parseFloat(resp.data.memory.usageRate))
      }).catch(err => {
        console.log("调用后端获取System Status出现异常：", err)
      })
    }
  }
}
