    ...
    <el-table :data="articleList">
      ...
      <el-table-column class-name="status-col" label="Deleted" width="90">
        <template v-slot="scope">
          <el-switch v-model="scope.row.deleted" @change="changeArticleStatus(scope.row)"></el-switch>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    deleteBlogById(id) {
      this.$confirm('It will delete this Article<strong style="color: red"> and all associated Data</strong>, Continue?', 'Notice', {
        confirmButtonText: 'Confirm',
        cancelButtonText: 'Cancel',
        type: 'warning',
        dangerouslyUseHTMLString: true
      }).then(() => {
        articleApi.delReal(id).then(res => {
